      ...
      <div className="max-w-7xl mx-auto px-10 grid md:grid-cols-12 gap-16 mb-24">
        ...
        <div className="md:col-span-3 lg:col-span-2">
          ...
          <ul className="space-y-4">
            {['About', 'Services', 'Team', 'Careers', 'Contact'].map((link) => (
              <li key={link}>
                <Link href={link === 'Home' ? '/' : `/${link.toLowerCase()}`} className="text-slate-500 hover:text-accentBlue text-sm font-bold transition-colors">
                  {link}
                </Link>
              </li>
            ))}
